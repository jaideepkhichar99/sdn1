...
def Switchlinkinfo(data, s):
    ...
    for i in data:
        ...
        # print("source ye hai " , source)
        # print("source type ye hai " ,str(type(source)))
        sourceTemp = str(source).split(":")[7]
        destinationTemp = str(destination).split(":")[7]

        # sourceTemp = "01"
        # destinationTemp = "04"
        sourceTemp = sourceTemp[:-1]
        destinationTemp = destinationTemp[:-1]
        latency = str(i['latency'])
        print('\nsource:      84-> ', sourceTemp)
        print('destination: 85-> ', destinationTemp)
        G.add_edge(int((sourceTemp), 16), int(destinationTemp, 16))
        # G.add_edge(int(destinationTemp, 16),int((sourceTemp), 16))
        # nx.add_path(G, [0, 10, 2])
        print('G is in 87-> ', G.number_of_edges())
        print('G is in 88-> ', G)
        # nx.draw(G)
        ...
        print('linkports in 100->', linkPorts)
...
def computeRoute():
    pathKey = ""
    nodeList = []
    src = int(str(switch[h1]).split(":", 7)[7], 16)
    dst = int(str(switch[h2]).split(":", 7)[7], 16)
    print("source is:122->", src)
    print("destination is:123->", dst)
    print("G is: 124->", G)
    # print([p for p in nx.all_shortest_paths(G, source=src, target=dst)])
...
def fetchLinkCost():
    ...
    global portKey
    global cost
    print("link ports in 168->", linkPorts)
    ...
    for key in path:
        ...
        for link in path[key]:
            ...
            if srcShortID == temp:
                ...
                print("port portKey is:", portKey)
                portNumber = linkPorts[portKey].split("::")[0]
                stats = "http://localhost:8080/wm/statistics/bandwidth/" + \
                    src + "/" + portNumber + "/json"
                fetchResponse(stats, "costcompute")
                srcShortID = temp
                src = link
        portKey = start.split(":")[7] + "::" + mid + \
            "::" + switch[h1].split(":")[7]
...
def addFlow():
    ...
    for currentNode in range(0, len(bestPath)):
        if previousNode == bestPath[currentNode].split(":")[7]:
            ...
            flowRule(bestPath[currentNode].split(":")[7], flowCount, str(
                inPort), str(outPort), staticFlowURL)
            flowCount = flowCount + 2
...
# Method to compute latency of every switcha
...
switch = {
    '10.0.0.1':	"00:00:00:00:00:00:00:01",
    '10.0.0.2':	"00:00:00:00:00:00:00:01",
    '10.0.0.3':	"00:00:00:00:00:00:00:02",
    '10.0.0.4':	"00:00:00:00:00:00:00:02",
    '10.0.0.5':	"00:00:00:00:00:00:00:03",
    '10.0.0.6':	"00:00:00:00:00:00:00:03",
    '10.0.0.7':	"00:00:00:00:00:00:00:04",
    '10.0.0.8':	"00:00:00:00:00:00:00:04",
    '10.0.0.9':	"00:00:00:00:00:00:00:05",
    '10.0.0.10': "00:00:00:00:00:00:00:06",
    '10.0.0.11': "00:00:00:00:00:00:00:07",
    '10.0.0.12': "00:00:00:00:00:00:00:08"
}
